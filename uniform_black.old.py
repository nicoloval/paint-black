...
if __name__ == "__main__":
    ...
    df["address_id"] = [catch(a, am) for a in df.address]
    c_indices = addr_cluster_map[df.address_id[~np.isnan(df.address_id)].astype(int)]
    clust_is_black[c_indices] = True

    df.to_csv(f"{options.output_folder}/ground_truth_addr_id.csv")
    zarr.save(f"{options.output_folder}/cluster_is_black.zarr", clust_is_black)
    del clust_is_black 
